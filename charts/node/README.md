...
- The pruning flag is now using `--blocks-pruning` which starts from polkadot version 0.928
...
| node.chainData.pruning | int | `1000` | Set the amount of blocks to retain. If set to 0 archive node will be run. If --pruning flags is used, set this to false. |
...
| node.collatorRelayChain.chainData.pruning | int | `1000` | Set the amount of blocks to retain for the collator relay-chain database.  If set to 0 archive node will be run. If --pruning flags is used, set this to false. |

...
| node.perNodeServices | object | `{"apiService":{"annotations":{},"enabled":true,"externalDns":{"customPrefix":"","enabled":false,"hostname":"example.com","ttl":300},"externalTrafficPolicy":"Cluster","extraPorts":[],"httpPort":9933,"prometheusPort":9615,"relayChainPrometheusPort":9625,"rpcPort":9944,"type":"ClusterIP","wsPort":9955},"paraP2pService":{"annotations":{},"enabled":false,"externalDns":{"customPrefix":"","enabled":false,"hostname":"example.com","ttl":300},"externalTrafficPolicy":"Cluster","extraPorts":[],"port":30334,"publishUnreadyAddresses":true,"type":"NodePort","webrtc":{"annotations":{},"enabled":false,"externalTrafficPolicy":"Cluster","port":30334,"type":"NodePort"},"ws":{"enabled":false,"port":30335}},"relayP2pService":{"annotations":{},"enabled":false,"externalDns":{"customPrefix":"","enabled":false,"hostname":"example.com","ttl":300},"externalTrafficPolicy":"Cluster","extraPorts":[],"port":30333,"publishUnreadyAddresses":true,"type":"NodePort","webrtc":{"annotations":{},"enabled":false,"externalTrafficPolicy":"Cluster","port":30333,"type":"NodePort"},"ws":{"enabled":false,"port":30334}},"setPublicAddressToExternalIp":{"autodiscoveryFix":false,"enabled":false,"ipRetrievalServiceUrl":"https://ifconfig.io"}}` | Configuration of individual services of the node |
...
| node.vault | object | `{"authConfigPath":null,"authConfigServiceAccount":null,"authConfigType":null,"authPath":null,"authRole":null,"authType":null,"keys":{},"nodeKey":{},"projectedToken":{"audience":"vault","enabled":false,"expirationSeconds":600,"mountPath":"/var/run/secrets/vault"}}` | Component to inject secrets via annotation of Hashicorp Vault ref: https://www.vaultproject.io/docs/platform/k8s/injector/annotations |
| node.vault.authConfigPath | string | `nil` | Configures the auth-config-path annotation (file the Vault Agent reads the JWT from). Used by the `jwt` auth method. When left empty and `projectedToken.enabled` is true, it defaults to `<projectedToken.mountPath>/token`. |
...
| node.vault.projectedToken | object | `{"audience":"vault","enabled":false,"expirationSeconds":600,"mountPath":"/var/run/secrets/vault"}` | Project a ServiceAccount token with a custom audience into the Vault Agent. Required for `authType: jwt` (e.g. authenticating to a Vault in another cluster |
| node.vault.projectedToken.audience | string | `"vault"` | Audience the token is scoped to. Must match the Vault JWT role's `bound_audiences`. |
| node.vault.projectedToken.enabled | bool | `false` | Enable the projected ServiceAccount token volume and wire it into the Vault Agent. |
| node.vault.projectedToken.expirationSeconds | int | `600` | Token lifetime in seconds (the kubelet rotates it before expiry). |
| node.vault.projectedToken.mountPath | string | `"/var/run/secrets/vault"` | Directory the token is mounted at; the JWT is at `<mountPath>/token`. |

...
| node.collatorRelayChain.customChainspecPath | string | `""` | Path to the file containing the chainspec of the collator relay-chain Set to /relaychain-data to use additional volume |
...
| node.externalRelayChain | object | `{"enabled":false,"relayChainRpcUrls":[]}` | EXPERIMENTAL!!! Run the collator node without a relay chain via external relay chain ref: https://github.com/paritytech/cumulus#external-relay-chain-node Enabling this option will disable the values of collatorRelayChain |
